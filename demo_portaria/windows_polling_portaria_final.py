...
class WindowsPollingService:
    def __init__(self):
        ...
        # SCRIPT PATH COM CAMINHO ABSOLUTO - VERSÃO SEM EMOJIS
        self.script_path = os.path.join(SCRIPT_DIR, 'test_hikcentral_demo_windows_sem_emoji.py')
        if not os.path.exists(self.script_path):
            print(f"[ERRO] Script não encontrado: {self.script_path}")
            print("[INFO] Certifique-se de que test_hikcentral_demo_windows_sem_emoji.py está na pasta")
            exit(1)
    ...
    def process_visitor(self, item):
        ...
        try:
            ...
            # Executar script de automação SEM EMOJIS
            cmd = [
                'python', 
                self.script_path,
